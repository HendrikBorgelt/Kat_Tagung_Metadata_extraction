...
MAIN_TTL = _os.path.join(_HERE, "ontology.ttl")
SECONDARY_TTL = _os.path.join(_HERE, "ontology.ttl")  # update if a second TTL is needed
OUTPUT_DIR = _os.path.join(_HERE, "doc")
WIDOCO_JAR = _os.path.join(_HERE, "widoco-1.4.25-jar-with-dependencies_JDK-17.jar")  # place JAR here

# Configuration files (optional - set to None if not using)
MAIN_CONFIG = _os.path.join(_ROOT, "data", "intermediate", "config_instances.json")  # or None
SECONDARY_CONFIG = None  # config_ontology.json no longer exists
...
def create_default_configs():
    ...
    create_config_file(DEFAULT_CONFIG_MAIN, _os.path.join(_ROOT, "data", "intermediate", "config_instances.json"))
    create_config_file(DEFAULT_CONFIG_SECONDARY, _os.path.join(_HERE, "config_ontology.json"))
