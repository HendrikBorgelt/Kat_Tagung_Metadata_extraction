...
def extract_bold_title(pdf_path, min_len=10, max_lines=5):
    """
    Extracts consecutive bold text lines at the top of the first page as title.
    """
    title_lines = []
            ...
            if isinstance(element, LTTextContainer):
                for text_line in element:
                    if not isinstance(text_line, LTTextLine):
                        continue
                    # Check if most characters are bold
                    bold_chars = 0
                    total_chars = 0
                    ...
                    if bold_ratio > 0.6:  # at least 60% bold
                        title_lines.append(line_text)
                    else:
                        # Stop when the first non-bold line appears after some bold ones
                        if title_lines:
                            return " ".join(title_lines)
        break  # Only analyze the first page
    return " ".join(title_lines)
...
def extract_metadata_from_text(text, pdf_path, fallback_title="Unknown Title"):
    ...
    #     else fallback_title
    # )
    metadata['dct:title'] = extract_bold_title(pdf_path) or fallback_title
