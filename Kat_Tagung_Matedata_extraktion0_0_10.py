...
class MetadataExtractorGUI:
    ...
    def create_widgets(self):
        ...
        # New buttons for Excel and folder selection
        ...
        # Store loaded Excel and folder info
        self.excel_path = None
        self.loaded_excel_df = pd.DataFrame()

        # --- The rest of create_widgets() stays the same ---
    ...
    def select_folder(self):
        ...
        self.folder_label.config(
            text=f"{len(self.pdf_files)} PDFs ready. Press 'Start Workflow' to begin."
        )
    ...
    def select_excel(self):
        ...
        # Create internal record mapping
    ...
    def start_workflow(self):
        ...
        start_index = 0  # default start at first PDF
        ...
        if self.loaded_excel_df is not None and not self.loaded_excel_df.empty:
            ...
            # find first PDF not yet in Excel
            ...
            messagebox.showinfo("Workflow Setup", f"No Excel loaded. Starting from first file.")

        # Set current index to the first unprocessed file
    ...
    def load_current_file(self):
        ...
        # 1. Get the authoritative (Excel/User-modified) data from internal memory
        # If the file is being seen for the very first time, this is an empty dict.
        existing_record = self.internal_records.get(name, {})

        # Ensure 'file_title' is always present from the filename
        if 'file_title' not in existing_record:
            existing_record['file_title'] = name

        # 2. Perform fresh automatic extraction (the 'recommendation')
        # This is done every time, but only used if existing_record is empty.
        ...
        # 3. Merge: Create the final metadata, prioritizing 'existing_record'.
        final_metadata = existing_record.copy()

        for key, rec_value in recommendation_data.items():
            current_value = final_metadata.get(key)

            # Check if the existing field is empty. We treat None, empty string, and pandas NaN as empty.
            is_empty = (
            ...
            # Check if the recommended value is actually useful (not empty itself)
            is_rec_useful = (
                    rec_value and
            ...
            # If the current field is empty AND the recommendation has data, use the recommendation.
            if is_empty and is_rec_useful:
                final_metadata[key] = rec_value

        # 4. Save the final (merged) record back to internal_records
        self.internal_records[name] = final_metadata
        metadata_to_load = final_metadata  # Rename for clarity with original variables

        # 5. Load the GUI from the final, merged metadata
        ...
        for key, field in self.metadata_fields.items():
            ...
            # Ensure lists (if your extract_metadata_from_text returns them) are joined to strings for the GUI Text widget
            if isinstance(value, list):
                value = "\n".join(value)

            # Ensure None/NaN values from Excel/Pandas are displayed as empty strings
            if pd.isna(value):
                value = ""

            field.insert("1.0", str(value))

        # Update label (using the 'text=' fix from the previous step)
    ...
    def extract_metadata_from_text(self, text, pdf_path, fallback_title="Unknown Title"):
        ...
        metadata['dct:publisher'] = "DECHEMA"  # static, not shown in GUI
        metadata['dcat:theme'] = "http://eurovoc.europa.eu/100142"  # static
        metadata['dct:type'] = "http://purl.org/spar/fabio/Abstract"  # static
        ...
        metadata['schema:Organization'] = ""  # new field, empty by default
        return metadata
    ...
    def go_to_pdf(self):
        """Jump to a specific PDF index entered by the user."""
        if not self.pdf_files:
    ...
    def collect_gui_metadata(self):
        """Gathers all metadata from the GUI text fields and returns a dictionary."""
        collected_data = {}
        for key, field in self.metadata_fields.items():
            # Get content from the Text widget from line 1, character 0 to the end,
            # minus the final newline. Then strip whitespace.
            value = field.get("1.0", tk.END).strip()

            # All fields are now stored as a single string
            collected_data[key] = value if value else ""

        # Ensure 'file_title' is included from the current file's basename
    ...
    def save_current_metadata(self):
        """Save GUI input into internal memory if user modified something."""
        if not hasattr(self, 'pdf_files') or not self.pdf_files:
            return
        ...
        new_data = self.collect_gui_metadata()  # ← assume this function gathers the fields from GUI widgets
    ...
    def finish_and_save(self):
        """Save internal memory back to Excel, preserving untouched rows."""
        ...
        # If Excel was loaded, reuse that path and columns
        ...
        messagebox.showinfo("Saved", f"Saved to {os.path.basename(save_path)}.\n"
