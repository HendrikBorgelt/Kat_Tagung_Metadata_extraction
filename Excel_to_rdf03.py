...
def name_to_uri(name: str) -> URIRef:
    ...
    clean = name.strip()
    clean = re.sub(r"[^A-Za-z0-9\s._-]", "", clean)  # remove special chars
    clean = clean.replace(".", "").replace(" ", "_")
    return EX[quote(clean)]  # URL-safe encoding for non-ASCII
    ...
    for col_name, value in row.items():
        if pd.notna(value):
            ...
            for entry in entries:
                ...
                # --- Handle FOAF Agents ---
                if col_name.lower().startswith("foaf") and "agent" in col_name.lower():
                    agent_uri = name_to_uri(entry)
                    g.add((abstract_uri, pred, agent_uri))
                    g.add((agent_uri, RDF.type, FOAF.Agent))
                    g.add((agent_uri, RDFS.label, Literal(entry.strip())))
