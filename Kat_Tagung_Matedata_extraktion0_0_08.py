...
class MetadataExtractorGUI:
    ...
    def select_folder(self):
        ...
        self.folder_label.config(
            text=f"{len(self.pdf_files)} PDFs found in folder (ready for workflow start)."
        )
        self.current_index = 0
        self.records = []
        self.current_metadata = None
        self.file_label.config(text="Folder loaded — press 'Start Workflow' to begin.")

    def select_excel(self):
        excel_path = filedialog.askopenfilename(
            title="Select existing Excel metadata file",
            filetypes=[("Excel files", "*.xlsx *.xls")]
        )
        if excel_path:
            try:
                self.loaded_excel_df = pd.read_excel(excel_path)
                self.excel_path = excel_path
                self.excel_label.config(text=f"Loaded: {os.path.basename(excel_path)}")
                messagebox.showinfo("Excel Loaded", f"Loaded {len(self.loaded_excel_df)} existing records.")
    ...
    def start_workflow(self):
        ...
        if self.loaded_excel_df is not None and not self.loaded_excel_df.empty:
            # Ensure file_title column exists
            if "file_title" not in self.loaded_excel_df.columns:
                messagebox.showerror("Excel Error", "The loaded Excel file has no 'file_title' column.")
                return

            existing_titles = set(self.loaded_excel_df['file_title'].astype(str).str.strip())

            # Separate PDFs into new and already processed ones
            new_files = []
            existing_files = []

            for f in self.pdf_files:
                basename = os.path.splitext(os.path.basename(f))[0]
                if basename in existing_titles:
                    existing_files.append(f)
                else:
                    new_files.append(f)

            # Reorder: new first, existing last
            self.pdf_files = new_files + existing_files

            messagebox.showinfo(
                "Workflow Setup",
                f"{len(new_files)} new PDFs first, {len(existing_files)} already known PDFs last."
            )
        else:
            messagebox.showinfo("Workflow Setup", f"{len(self.pdf_files)} PDFs to process (no Excel loaded).")

        if not self.pdf_files:
            messagebox.showinfo("Nothing to Process", "No PDF files found.")
            return

        self.folder_label.config(text=f"{len(self.pdf_files)} PDFs ready for review.")
        self.current_index = 0
        self.records = []
    ...
    def finish_and_save(self):
        ...
        try:
            ...
            if self.excel_path and not self.loaded_excel_df.empty:
                # Merge with existing Excel
                combined_df = pd.concat([self.loaded_excel_df, new_df], ignore_index=True)
