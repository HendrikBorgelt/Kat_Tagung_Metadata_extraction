...
# === Color Configuration ===
CLASS_COLORS = {
    'person': '#4a7c59',      # Medium dark green
    'organization': '#87ceeb', # Light blue (sky blue)
    'abstract': '#000080'      # Navy blue
}
...
def extract_person_subgraph(g, person_uri):
    ...
    # Add person's datatype properties as attributes
    for pred, obj in g.predicate_objects(person_uri):
        if isinstance(obj, Literal) and pred != FOAF.name:
            entities[str(person_uri)]['attributes'].append({
    ...
    for org_uri in g.objects(person_uri, EX.hasAffiliation):
        ...
        # Add organization's datatype properties as attributes
        for pred, obj in g.predicate_objects(org_uri):
            if isinstance(obj, Literal) and pred not in [SCHEMA.name, RDFS.label]:
                entities[str(org_uri)]['attributes'].append({
                    'key': str(pred).split('/')[-1].split('#')[-1],
                    'value': str(obj)
                })
    ...
    for abstract_uri in g.subjects(EX.hasAuthor, person_uri):
        ...
        # Add abstract's datatype properties as attributes
        for pred, obj in g.predicate_objects(abstract_uri):
            if isinstance(obj, Literal) and pred not in [DCT.title, RDFS.label]:
                entities[str(abstract_uri)]['attributes'].append({
                    'key': str(pred).split('/')[-1].split('#')[-1],
                    'value': str(obj)[:200]
                })
...
def extract_abstract_subgraph(g, abstract_uri):
    ...
    for author_uri in g.objects(abstract_uri, EX.hasAuthor):
        ...
        # Add author's datatype properties as attributes
        for pred, obj in g.predicate_objects(author_uri):
            if isinstance(obj, Literal) and pred not in [FOAF.name, RDFS.label]:
                entities[str(author_uri)]['attributes'].append({
                    'key': str(pred).split('/')[-1].split('#')[-1],
                    'value': str(obj)
                })
...
def generate_webvowl_json(classes, properties, class_attrs):
    ...
    base_classes = [
        ...
        },
        {
            ...
            "label": "Organization",
            "attributes": ["color"],
            "color": CLASS_COLORS['organization']
        }
    ]
    ...
    # Update instances to reference base classes and add colors
    for cls in classes:
        if 'person' in cls['id']:
            cls['type'] = 'owl:Class'
            cls['subClassOf'] = 'foaf_Person'
            cls['attributes'] = cls.get('attributes', []) + ['color']
            cls['color'] = CLASS_COLORS['person']
        elif 'abstract' in cls['id']:
            cls['type'] = 'owl:Class'
            cls['subClassOf'] = 'ex_Abstract'
            cls['attributes'] = cls.get('attributes', []) + ['color']
            cls['color'] = CLASS_COLORS['abstract']
        elif 'org' in cls['id']:
            cls['type'] = 'owl:Class'
            cls['subClassOf'] = 'schema_Organization'
            cls['attributes'] = cls.get('attributes', []) + ['color']
            cls['color'] = CLASS_COLORS['organization']
    ...
    return {
        "_comment": "Generated by Knowledge Graph Visualizer with Custom Colors",
        "header": {
...
class KnowledgeGraphVisualizerGUI:
    def __init__(self, root):
        ...
        self.output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'assets', 'visualizations')
